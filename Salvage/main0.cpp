...
CONST_maxKidCount = 6, CONST_kidCount = 2, CONST_maxCarCount = 3, CONST_animationFrameCount = 3, CONST_maxExplosions = 20, CONST_playerCarCount = 2, CONST_numberOfSongs[] = { 0, 4, 3, 4 }, CONST_maxNumberOfSongs = 4, CONST_hudiconCount = 10,
...
CONST_maxVendorItems = 3, CONST_defaultSeaLevel = 1040, CONST_maxFishes = 6, yFlane[] = { 250, 350, 450, 550, 650, 750 }, CONST_maxFishFrames = 3, CONST_fishSpawnChance[] = { 200, 75, 50, 20, 20, 20}, CONST_fishFrames[] = { 2, 3, 3, 3, 3, 3 },
CONST_maxFishShadows = 4, CONST_fishShadows[] = { 1, 1, 1, 1, 4, 1 }, CONST_numberOfASongs[] = { 0, 0, 0, 4 }, CONST_maxMoneybags = 1, CONST_moneybagValue[] = { 3000 }, CONST_oyeNumber = 8, CONST_itemPrice[] = { 120, 40, 80 };
...
usedASongsNumber[CONST_maxDrinks], playerMoney, oyeLevel, oyeTime, oyeOpacity, immunityTime, vendorQuotesNumber, vendorQuotesNumberGood, vendorQuotesNumberBad, donutOpacity, donutTime;
long long score, highscore[CONST_maxDrinks];
float roadSpeed, policeMoveX, policeMoveY, carVelocityX, carVelocityY, shx, shy, imagineVolume, bombShadowScale, donutScale;
...
newHighscoreReachedEnd, lennonActive, hardmode, speedMilestone[10], slowdownEquipped, planeStart, bombAlarmActive, underwaterActive, usedFlane[6], usedASong[CONST_maxDrinks][CONST_maxNumberOfSongs], tpActive,
leftPressed2, rightPressed2, upPressed2, downPressed2, bellfishEquipped, cPressed, jPressed, kPressed, lPressed, seashellEquipped, donutEquipped, cornEquipped, summonWhale;
...
CapsuleBrokenTex, OyeTex[CONST_oyeNumber], CornAuraTex, ImmunityAuraTex, VendorWindowTex, VendorWindowTexGood, VendorWindowTexBad, DonutTex;
...
LoadingSpr, CarAuraSpr, DarknessSpr, CapsuleSpr, CapsuleBrokenSpr, OyeSpr, CornAuraSpr, ImmunityAuraSpr, VendorWindowSpr, DonutSpr;
sf::Font libre, typewriter, scorefont, terar, miology, whalefont;
sf::Text txtTitleScreen, selectDrink, useArrows, scoreText, quoteText, highscoreMenu, newHighscore, bonusScoreText, jackpotText, endscreenText[CONST_maxEndscreenText], newHighscoreEnd, whaleQuoteText, moneyText, vendorQuoteText,
vendorMoneyText;
...
WhaleVoiceSoundB, MoneySoundB, BellfishSoundB, SeacallSoundB, VendorGoodSoundB, VendorBadSoundB, VendorNewSoundB, BloodSoundB;
...
BiteSound, GoldfishSound, ReturnfishSound, AnglerSound, JellyfishSound, WhalethrowSound, WhaleVoiceSound, MoneySound, BellfishSound, SeacallSound, VendorGoodSound, VendorBadSound, VendorNewSound;
IndexCar ieCar[10];
...
void LoadStuff()
{
    ...
    selectDrink.setPosition(40, 130);
    selectDrink.setOutlineColor(sf::Color(0, 0, 0, 128));
    selectDrink.setOutlineThickness(5);

    useArrows.setFont(typewriter);
    useArrows.setString("Use Left / Right arrows to navigate and Space to select!");
    useArrows.setCharacterSize(26);
    useArrows.setPosition(1200, 130);
    useArrows.setRotation(22);
    useArrows.setOutlineColor(sf::Color(0, 0, 0, 128));
    useArrows.setOutlineThickness(3);
...
int main()
{
    ...
    srand(time(NULL));
    //sf::RenderWindow window(sf::VideoMode(Wx, Wy), "Drunk Driving Simulator", sf::Style::Fullscreen);
    sf::RenderWindow window(sf::VideoMode(Wx, Wy), "Drunk Driving Simulator");
    window.setFramerateLimit(60);
    window.setVerticalSyncEnabled(true);
    window.setMouseCursorVisible(false);
    
    sf::Listener::setGlobalVolume(25);
    ...
    while (window.isOpen())
    {
        ...
        keyPressed = leftPressed = rightPressed = spacePressed = 0;
        ...
        if (status == 1)                                                 // driving real !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        {
            ...
            if (activeEvent == 3)                        // blizzard event
            {
                ...
                else if (eventTimeFrame == 100)
                {
                    //FogSpr.setColor(sf::Color(255, 255, 255, 255));
                    //FogSpr1.setColor(sf::Color(255, 255, 255, 255));
                    //SnowstormSpr.setColor(sf::Color(255, 255, 255, 255));
                    //SnowstormSpr1.setColor(sf::Color(255, 255, 255, 255));
                }
                        ...
                        for (l = 0; l < 3; l++)
                        {
                            ...
                            if (i != -1 and j != -1 and enemyCar[i][0].active[j])
                                if (P1.x + enemyCar[i][0].size.x >= target[i].x - 2 * target[i].size.x and P1.x <= target[i].x + 2 * target[i].size.x and P1.y - enemyCar[i][0].size.y / 2 <= target[i].y + 2 * target[i].size.y and P1.y - enemyCar[i][0].size.y / 2 >= target[i].y - 2 * target[i].size.y)
                                {
                                    ...
                                    int l1 = 0;
                                    while (explosion[l1].active)
                                        l1++;

                                    if (!explosion[l1].tex.loadFromFile("Assets/Textures/Effects/Explosion/frame25.png"));
                                    explosion[l].sound.play();
                                    explosion[l1].spr.setTexture(explosion[l1].tex);
                                    explosion[l1].scale = 4;
                                    explosion[l1].spr.setScale(explosion[l1].scale, explosion[l1].scale);
                                    explosion[l1].spr.setPosition(P1.x, P1.y - 2 * enemyCar[i][0].size.y);
                                    explosion[l1].frame = -1;
                                    explosion[l1].active = 1;
                        ...
                        if (i1 != -1 and i2 != -1 and enemyCar[i1][0].active[j1] and enemyCar[i2][0].active[j2])
                            if (enemyCar[i1][0].Lane[j1] == enemyCar[i2][0].Lane[j2] and std::max(P1.x, P2.x) <= std::min(P1.x + enemyCar[i1][0].size.x, P2.x + enemyCar[i2][0].size.x))
                            {
                                ...
                                if (i1 != 9)
                                {
                                    ...
                                    explosion[l].scale = 4;
                                    explosion[l].spr.setScale(explosion[l].scale, explosion[l].scale);
                                    explosion[l].spr.setPosition(P1.x, P1.y - 2 * enemyCar[i1][0].size.y);
                                    explosion[l].frame = -1;
                                    explosion[l].active = 1;
                                ...
                                if (i2 != 9)
                                {
                                    ...
                                    explosion[l].scale = 4;
                                    explosion[l].spr.setScale(explosion[l].scale, explosion[l].scale);
                                    explosion[l].spr.setPosition(P2.x, P2.y - 2 * enemyCar[i2][0].size.y);
                                    explosion[l].frame = -1;
                                    explosion[l].active = 1;
                    ...
                    for (int j = 0; j < 3; j++)
                        if (enemyCar[i][0].active[j])
                        {
                            ...
                            if (p2.y - 40 >= seaLevel)
                            {
                                ...
                                explosion[l].scale = 4;
                                explosion[l].spr.setScale(explosion[l].scale, explosion[l].scale);
                                explosion[l].spr.setPosition(p2.x, p2.y - 2 * enemyCar[i][0].size.y);
                                explosion[l].frame = -1;
                                explosion[l].active = 1;
            ...
            if (!enemyCar[6][0].active[0])
            {
                ...
                {
                    imagineVolume *= 0.85f;
                    ImagineSound.setVolume(imagineVolume);
                    imagineTime++;
                }
            }
            else
                ...
                for (int l = 0; l < 3; l++)
                {
                    ...
                    if (i != -1 and j != -1 and enemyCar[i][0].active[j])
                        if (p2.x >= P1.x and p2.x <= P1.x + enemyCar[i][0].size.x and p2.y <= P1.y - enemyCar[i][0].size.y / 6 and p2.y >= P1.y - 5 * enemyCar[i][0].size.y / 6)
                        {
                            ...
                            explosion[l1].spr.setScale(explosion[l1].scale, explosion[l1].scale);
                            if (i == 9)
                                explosion[l1].spr.setPosition(P1.x + enemyCar[i][0].size.x / 4, P1.y - 3 * enemyCar[i][0].size.y / 2);
                            else
                                explosion[l1].spr.setPosition(P1.x, P1.y - 2 * enemyCar[i][0].size.y);
                            explosion[l1].frame = -1;
                            explosion[l1].active = 1;
            ...
            if (drink == 3 and !vendor.active and !underwaterActive and activeEvent != 6)                                                    // vendor spawn
            {
                ...
                if (Rand < 50)
                {
                    ...
                    Rand = rand() % CONST_maxVendorItems;
                    vendor.item = Rand;
                ...
                else
                {
                    if (tideActive)
                    {
                        ...
                        else
                        {
                            ...
                            alternateVolume = 0;
                            AlternateSong.play();
                            AlternateSong.setVolume(alternateVolume);
                            CurrentSong.pause();
                        }
                    }
                    else
            ...
            if (underwaterActive and transitionTime == 0)                                                          
            {
                ...
                if ((underwaterTime >= 900 or summonWhale) and !whale.active)                 // EDIT before release!!!!!!!
                {
                    Rand = rand() % 10000;
                    //Rand = 0;
            ...
            {
                if (drink == 3 and !moneybag[i].active and !whale.active)
                {
                    Rand = rand() % 10000;
...
;
            ...
            window.draw(RoadSpr);                                             // drawing stuff time!
            ...
            if (!whale.carDespawn or whale.onShore)
            {
                object[objectCount].spr = car[playerCar][playerCarFrame].spr;
                object[objectCount].y = p1.y + car[playerCar][playerCarFrame].size.y / 2;
                object[objectCount].priority = PRIORITY_car;
                objectCount++;
            }
            ...
            for (int l = 0; l < 3; l++)
            {
                ...
                if (i != -1)
                {
                    if (enemyCar[i][0].active[j])
                    {
                        object[objectCount].spr = enemyCar[i][playerCarFrame].spr[j];
                        object[objectCount].y = yLane[enemyCar[i][0].Lane[j]];
                        object[objectCount].priority = PRIORITY_car;
                        objectCount++;
                    }
                }
            ...
            for (int i = 0; i < CONST_maxBananaCount; i++)
                if (banana.active[i])
                {
                    object[objectCount].spr = banana.spr[i];
                    object[objectCount].y = banana.spr[i].getPosition().y;
                    object[objectCount].priority = PRIORITY_banana;
                    objectCount++;
                }
            ...
            for (int i = 0; i < CONST_kidCount; i++)
                for (int j = 0; j < 3; j++)
                    if (kid[i].active[j])
                    {
                        object[objectCount].spr = kid[i].spr[j];
                        object[objectCount].y = kid[i].spr[j].getPosition().y + kid[i].size.y;
                        object[objectCount].priority = PRIORITY_kid;
                        objectCount++;
                    }
            ...
            for (int i = 0; i < CONST_maxExplosions; i++)
                if (explosion[i].active)
                {
                    object[objectCount].spr = explosion[i].spr;
                    object[objectCount].y = explosion[i].spr.getPosition().y + explosion[i].scale * explosion[i].tex.getSize().y;
                    object[objectCount].priority = PRIORITY_explosion;
                    objectCount++;
                }
            ...
            for (int i = 0; i < CONST_maxExplosions; i++)
                if (blood[i].active)
                {
                    object[objectCount].spr = blood[i].spr;
                    object[objectCount].y = blood[i].spr.getPosition().y + blood[i].tex.getSize().y / 2;
                    object[objectCount].priority = PRIORITY_explosion;
                    objectCount++;
                }
            ...
            if (carChaseActive)
            {
                object[objectCount].spr = policeCar[playerCarFrame].spr;
                object[objectCount].y = policeCar[playerCarFrame].spr.getPosition().y;
                object[objectCount].priority = PRIORITY_car;
                objectCount++;
            }
            ...
            if (shot[0].active)
            {
                object[objectCount].spr = shot[playerCarFrame].spr;
                object[objectCount].y = shot[playerCarFrame].spr.getPosition().y + shot[0].size.y / 2;
                object[objectCount].priority = PRIORITY_shot;
                objectCount++;
            }

            if (ammobox[0].active)
            {
                object[objectCount].spr = ammobox[playerCarFrame].spr;
                object[objectCount].y = ammobox[playerCarFrame].spr.getPosition().y + ammobox[0].size.y / 2;
                object[objectCount].priority = PRIORITY_shot;
                objectCount++;
            }
            ...
            {
                object[objectCount].spr = slowdown[playerCarFrame].spr;
                object[objectCount].y = slowdown[playerCarFrame].spr.getPosition().y + slowdown[0].size.y / 2;
                object[objectCount].priority = PRIORITY_shot;
                objectCount++;
            }
            ...
            if (rpg[0].active)
            {
                object[objectCount].spr = rpg[playerCarFrame].spr;
                object[objectCount].y = rpg[playerCarFrame].spr.getPosition().y + rpg[0].size.y / 2;
                object[objectCount].priority = PRIORITY_rpg;
                objectCount++;
            }

            if (rocket.active)
            {
                object[objectCount].spr = rocket.spr;
                object[objectCount].y = rocket.spr.getPosition().y + rocket.size.y / 2;
                object[objectCount].priority = PRIORITY_car;
                objectCount++;
            }
            ...
            for (int i = 0; i < CONST_maxBullets; i++)
            {
                if (bullet[i].active)
                {
                    object[objectCount].spr = bullet[i].spr;
                    object[objectCount].y = bullet[i].spr.getPosition().y + bullet[i].size.y / 2;
                    object[objectCount].priority = PRIORITY_car;
                    objectCount++;
                }
            }
            ...
            for (int i = 0; i < CONST_maxBalls; i++)
            {
                if (ball[i].active)
                {
                    object[objectCount].spr = ball[i].spr;
                    object[objectCount].y = yLane[ball[i].Lane];
                    object[objectCount].priority = PRIORITY_car;
                    objectCount++;
                }
            }
            ...
            if (vendor.active)
            {
                object[objectCount].spr = vendor.spr;
                object[objectCount].y = vendor.spr.getPosition().y;
                object[objectCount].priority = PRIORITY_car;
                objectCount++;

                if (!vendor.good and !vendor.bad)
                {
                    object[objectCount].spr = vendor.itemSpr;
                    object[objectCount].y = vendor.spr.getPosition().y;
                    object[objectCount].priority = PRIORITY_car;
                    objectCount++;

                    object[objectCount].spr = vendor.auraSpr;
                    object[objectCount].y = vendor.spr.getPosition().y;
                    object[objectCount].priority = PRIORITY_kid;
                    objectCount++;
                }
            ...
            for (int i = 0; i < CONST_maxFishes; i++)
                for (int j = 0; j < 4; j++)
                    if (fish[i][j].active)
                    {
                        object[objectCount].spr = fish[i][j].spr;
                        object[objectCount].y = fish[i][j].spr.getPosition().y + 150;
                        object[objectCount].priority = PRIORITY_car;
                        objectCount++;
                    }
            ...
            if (goldfishTime < 0)
            {
                object[objectCount].spr = CarAuraSpr;
                object[objectCount].y = CarAuraSpr.getPosition().y + CarAuraTex.getSize().y / 2;
                object[objectCount].priority = PRIORITY_car + 1;
                objectCount++;
            }

            if (cornEquipped and !whale.carDespawn)
            {
                object[objectCount].spr = CornAuraSpr;
                object[objectCount].y = CornAuraSpr.getPosition().y + CornAuraTex.getSize().y / 2;
                object[objectCount].priority = PRIORITY_car + 1;
                objectCount++;
            }
            ...
            if (immunityTime < 0)
            {
                object[objectCount].spr = ImmunityAuraSpr;
                object[objectCount].y = ImmunityAuraSpr.getPosition().y + ImmunityAuraTex.getSize().y / 2;
                object[objectCount].priority = PRIORITY_car + 2;
                objectCount++;
            }
            ...
            for (int i = 0; i < CONST_maxMoneybags; i++)
                if (moneybag[i].active)
                {
                    object[objectCount].spr = moneybag[i].spr;
                    object[objectCount].y = moneybag[i].shadowSpr.getPosition().y;
                    object[objectCount].priority = PRIORITY_shot;
                    objectCount++;
                }
            ...
            if (gunflashTime < 0)
            {
                ...
                object[objectCount].spr = GunflashSpr;
                object[objectCount].y = p1.y + car[playerCar][playerCarFrame].size.y / 2;
                object[objectCount].priority = PRIORITY_car;
                objectCount++;
            }
        ...
        else if (status == 2)                                                          // death !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        {
            ...
            for (int i = 0; i < CONST_kidCount; i++)
                for (int j = 0; j < 3; j++)
                    ...
                    {
                        object[objectCount].spr = kid[i].spr[j];
                        object[objectCount].y = kid[i].spr[j].getPosition().y;
                        object[objectCount].priority = PRIORITY_kid;
                        objectCount++;
                    }
        ...
        else if (status == 3)                                                            // drink selection prep !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        {
        ...
        else if (status == 4)                                                           // driving prep !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        {
            ...
            for (int i = 0; i < 3; i++)
                car[playerCar][i].spr.setPosition(Wx / 4, Wy / 2);
            ...
            endscreenAirstrikeCounter = endscreenBlizzardCounter = 0;
            ...
            playerMoney = donutTime = 0;
            for (int i = 0; i < CONST_maxMoneybags; i++)
                moneybag[i].active = 0;

            summonWhale = bellfishEquipped = donutEquipped = cornEquipped = seashellEquipped = 0;

            status = 1;
        }
        else if (status == 5)                                                                       // drink selection real !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        {
            if (flickerTime < 0)
            {
                flickerTime++;
